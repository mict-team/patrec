...
import VTreeview from "./MTreeview";
...
export default {
  ...
  computed: {
    ...
    allDescendantLeafsSelected() {
      return (
        this.value.includes(this.items[this.keyField]) ||
        this.parentSelected ||
        (this.hasChildren &&
          this.hasSelection &&
          (this.children.every(child =>
            this.value.some(sel => sel === child[this.keyField])
          ) ||
            this.allDescendantLeafs.every(leaf =>
              this.value.some(sel => sel === leaf[this.keyField])
            )))
      );
    },
    ...
    hasChildren() {
      return (
        ...
        (!!this.items[this.childrenField] &&
          Array.isArray(this.items[this.childrenField]) &&
    ...
    leftPadding() {
      return (
        this.level * 24 +
        (this.expandable ? this.level * 24 : 0) -
        (this.expandable &&
        this.level > 0 &&
        (this.hasChildren || this.hasDynamicChildren)
          ? this.level * 24 + 4
          : 0) -
        (this.expandable &&
        this.level > 0 &&
    ...
    someDescendantLeafsSelected() {
      return (
        ...
        (this.children.some(child =>
          ...
          this.allDescendantLeafs.some(leaf =>
            this.value.some(sel => sel === leaf[this.keyField])
          ))
      );
    },
    selected() {
      return (
        this.select &&
        this.hasSelection &&
        this.value[this.value.length - 1] === this.items[this.keyField]
      );
    }
  },
  ...
  methods: {
    checkExpandChildren() {
      if (
        !this.hasDynamicChildren &&
        (!this.expandable || this.expandAll || (!this.root && this.level === 0))
      ) {
      ...
      return this.$createElement(
        VCheckbox,
        {
          props: {
            ...
            inputValue:
              this.parentSelected ||
              (this.hasChildren ? this.allDescendantLeafsSelected : this.value),
            value:
              this.parentSelected ||
              (this.hasChildren
                ? this.allDescendantLeafsSelected
                : this.items[this.keyField]),
      ...
      if (this.checkbox) {
        return this.$createElement(
          ...
          this.root || (this.checkbox && this.level > 0)
            ? [this.genExpandButton(), this.genCheckbox(), this.genToolbar()]
            : []
        );
      } else {
        return this.$createElement(
          ...
          [this.genRootLabel(), this.genToolbar()]
        );
      }
    },
    genRootLabel() {
    ...
    genChild(child, index) {
      return this.$createElement(VTreeview, {
        ...
        props: {
          ...
          parentSelected:
            this.parentSelected || this.allDescendantLeafsSelected,
          root: this.root,
          select: this.select,
          value: this.value
        }
    ...
    getChildrenArray(items) {
      if (Array.isArray(items)) {
        ...
        Array.isArray(items[this.childrenField])
      ) {
