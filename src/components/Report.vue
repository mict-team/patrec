            ...
            <div class="page-header">
              <v-layout row wrap>
                <v-flex xs8 row wrap>
                  ...
                  <v-flex xs5 style="color:red" v-if="me.acct_type==1">
                    Total: &#8358;{{getTotalSum | toDecimal}}
                  </v-flex>
                  <v-flex xs3 style="color:green">
...
export default {
  data() {
    return {
      headers: [
        ...
        { text: "Payee", align: "left", sortable: true, value: "name" },
        { text: "Card No", align: "left", sortable: true, value: "card_no" },
        { text: "Description", value: "desc", sortable: false, align: "left" },
        { text: "Amount", value: "amount", sortable: false, align: "left" },
        { text: "Discount", value: "discount", sortable: false, align: "left" },
        { text: "VAT", value: "vat", sortable: false, align: "left" },
      ...
      parameters: { from: "", to: "", code: "", collector: "", name: "", user:[] },
      modalFrom: false,
      modalTo: false,
      loadingDialog: false,
      viewReceipt:false,
  ...
  methods: {
    ...
    loadReport: function(){
      ...
      axios.post(this.domain + "/report", this.parameters)
      .then(response => {

          if(response.data.status>0){
            if(response.data.response.report.length>0){
              this.list=response.data.response.report;
              this.numOfRec=response.data.response.count;
            }else{
              this.list=[];
              this.numOfRec=0;
            }
