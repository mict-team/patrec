<template>
  <v-container>
    <v-toolbar row wrap>
      ...
      <v-flex xs3 row>
        <v-flex xs6
          ><button class="btn blue" @click="saveProduct">
            Save product
          </button></v-flex
        >
        <v-flex xs6
          ><button class="btn btn-info" @click="$router.push('upload-drug')">
            Upload items
          </button></v-flex
        >
      </v-flex>
...
export default {
  ...
  methods: {
    ...
    loadProduct: function(id) {
      this.get(`/drugs/get-product/${id}`)
        .then(response => {
          if (response.status > 0) {
            ////{"status":1,"message":null,"response":{"id":500,"outlet_id":1,"name":"Amisol Syrup","remark":null,"brand":5,"unit":"Bottle","category":4,"sku":33090,"product_type":0,"composite":null,"manage_stock":1,"alert_qty":5,"expiry_date":"","cost_price":0,"gain_margin_type":"","gain_margin":"","selling_price":1,"opening_stock":5,"available_qty":0,"qty_sold":0,"picture_type":null,"picture_path":null}}
            ...
            this.drugs = {
              ...
              brand: resp.brand,
              picture: `${resp.picture_type},${resp.picture_path}`,
              description: resp.remark,
              qty: resp.available_qty,
              alert_qty: resp.alert_qty,
              expiry_date: resp.expiry_date,
              selling_unit: resp.unit,
              make_opening_stock: resp.manage_stock,
              product_type: resp.product_type == 0 ? "normal" : "composite",
              products: resp.composite,
              cost_price: resp.cost_price,
              selling_price: resp.selling_price,
              gain_type: resp.gain_margin_type == 0 ? "0" : "%",
              gain_margin: resp.gain_margin
            };
          } else {
