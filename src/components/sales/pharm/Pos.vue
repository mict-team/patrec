          ...
          <div class="portlet-body">
            <div class="panel light scroller" style="height:935px !important;">
                <v-row style="height:auto !important; padding: 10px 10px;">
                  <v-col md="3" v-for="(product, index) in productsFilter" :key="index" @click="addProductToCart(product)" style="height: 134px;" :disabled="!isNegativeInventorySellable(product)">
                    <v-card class="thumbnail" style="margin-top:-10px;">
                      ...
                      <div class="amount">NGR{{ product.amount | toDecimal }}({{product.qty}})</div>
                    </v-card>
                  </v-col>
                </v-row>
            </div>
                    ...
                    <tbody>
                      ...
                      <tr v-if="settings.almona && settings.almona.allow_type">
                        <td colspan="2" style="text-align:; color: #000">
                          <v-flex xs12 row>
                            <v-flex xs3 class="pl-4 mt-2">Scheme: </v-flex>
                            <v-flex xs4>
                              <v-select
                                v-model="carts.type"
                                @change="changeType"
                      ...
                      <tr v-if="settings.almona && settings.almona.allow_discount">
                        <td colspan="2" style="width:90%;color:#000; text-align: left">
                          <v-flex xs12 row>
                            <v-flex xs3 class="pl-4 mt-2">{{ label }}</v-flex>
                            <v-flex xs7>
                            ...
                            <v-flex xs2 class="pl-4 mt-2" style="text-align: right">
                              &#8358;{{
                      ...
                      <tr v-if="settings && settings.almona.allow_vat">
...
export default {
  ...
  methods: {
    ...
    isNegativeInventorySellable: function(item){
      if(item.qty<1 && this.settings.pharmacy && this.settings.pharmacy.allow_negative_inventory){
        return true;
      }else{
        if(item.qty<item.alert_qty && this.settings.pharmacy && this.settings.pharmacy.allow_sales_for_items_lower_alert_qty){
          return true;
        }else{
          return false;
        }
    ...
    isAlertLevelItemsSellable: function(item){
      if(item.qty<item.alert_qty && this.settings.pharmacy && this.settings.pharmacy.allow_sales_for_items_lower_alert_qty){
        return true;
      }else{
        return false;
    ...
    innitialize: function() {
      ...
      this.post(`/pos/pharmacy`, payload)
      .then(resp => {
      ...
      .finally(() => {
        this.schemeData.push({id:0, name: 'No scheme', discount:100, status:1});
        for(let index in this.schemes){
          this.schemeData.push(this.schemes[index]);
        }
    ...
    paynowHandler: function(index) {
      ...
      this.getCode();
      this.getToday();
      this.carts.location=this.location.id;
      ...
      this.post(`/bills/pharm`, this.carts)
        .then(resp => {
          if (resp.status > 0) {
    ...
    updateProductQty: function(){
      ...
      console.log(query);
      //LS.remove("carts");
      //this.$router.push("sales/pharmacy/pharm/receipt/"+resp.response);//{ name: "receipt", id: resp.response });
    },
    openWaver: function() {
    ...
    changeType: function() {
      ...
      let index=this.schemeData.findIndex(x=>x.id==this.carts.type);
      if(index>-1){
        this.transType=this.schemeData[index];
      }
