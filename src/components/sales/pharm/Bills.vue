            ...
            <template v-else>
              <tbody v-if="getItems.length>0">
                <tr v-for="(pat, index) in getItems" :key="index">
                  ...
                  <td>{{ pat.gender }}</td>
                  <td>{{ pat.dob|toAge }}</td>
                  <td>{{fixDescription(pat.prescribtion)}}</td>
                  <td>
                    <v-chip class="ma-2" color="blue" text-color="white" @click="$router.push(`/pharmacy/billings/${pat.pat_med_id}/${pat.id}`)">
                      View prescribtion
                    </v-chip>
                  </td>
                </tr>
